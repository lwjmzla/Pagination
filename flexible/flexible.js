  ...
  _processRules(rules) {
    ...
    for (let i = 0; i < rules.length; i++) {
      ...
      if (declarations.length) {
        declarations.forEach((declaration, index) => {
          ...
          if (declaration.type === 'declaration' && nextDeclaration && nextDeclaration.type === 'comment') {
            const trimComment = nextDeclaration.comment.trim();
            // console.log(declaration.value); // 这是 冒号后的值: xxx
            // console.log(trimComment)  // hair img-flex那些
            if (pxRegExp.test(declaration.value)) {
              if (declaration.value === '0px') {
                ...
                // 遇到hair注释的
                // 添加0.5px的样式
                // console.log(rule.selectors); // [ '.login-container p' ]
              ...
              if (rule.selectors) {
                imgUrlList.push({
                  selectors: rule.selectors, // [ '.login-container p' ]
                  value: declaration.value // 这是 冒号后的值: xxx
                });
              }
    ...
    if (imgUrlList.length) {
      ...
      imgUrlList.forEach(item => {
        ...
        // console.log(splitByQuery); // [ 'url(/static/images/login/login-bg@3x.png)' ]
        rule.rules.push({
          type: 'rule',
